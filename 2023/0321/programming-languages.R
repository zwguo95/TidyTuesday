...
background <- image_read("computer-desktop-icon.png")
...
icon_data <- data.frame(ximage = rep(0, 4),
                        xlabel = rep(0.3, 4),
                        y = c(4,3,2,1),
                        labels = c("//", "#", ";", "--"),
                        image = images)
...
ggplot() +
  ...
  geom_text(data = icon_data, aes(x = xlabel, y = y, label = labels), vjust = 1, color = "white", size = 20) +
  geom_image(data = icon_data, aes(x = ximage, y = y, image = images), asp=1.5, size=0.05) +
  annotate(geom = "text", label = "1970s", x = 1, y = 0.6, color = "white", size = 16, family = "Roboto") +
  annotate(geom = "text", label = "1980s", x = 2, y = 0.6, color =  "white", size = 16, family = "Roboto") +
  annotate(geom = "text", label = "1990s", x = 3, y = 0.6, color = "white", size = 16, family = "Roboto") +
  annotate(geom = "text", label = "2000s", x = 4, y = 0.6, color = "white", size = 16, family = "Roboto") +
  ...
  theme(panel.background = element_rect(fill = "black", color = NA),
        ...
        plot.subtitle = element_textbox_simple(
          ...
          hjust = 0,
          size = 50,
          margin = margin(t = 10, l=20, r=20, b = 10)
        ),
        plot.caption = element_textbox_simple(
          lineheight = 0.4,
          colour = "#66FF00",
          size = 40,
          margin = margin(t = 5, l=20, b=10)
        ))
